...
def getSimiliarFiles(repoDir,strict,work_dir, sess):
    ...
    with open("./result/MeasureIndex.csv","r") as f:
        ...
        for line in lines:
            ...
            if "vulFileVersion" in fileName:
                ...
                file_hash_line_number_to_index_dict[file_hash+"__split__"+endLine.strip()]=id
        ...
        for ele in CVE_dict[CVE].keys():
            for name in CVE_dict[CVE][ele].keys():
                if CVE_dict[CVE][ele][name]["lineEnd"] - CVE_dict[CVE][ele][name]["lineStart"] <= 2:
                    continue
                if ele+"__split__"+str(CVE_dict[CVE][ele][name]["lineEnd"]) in file_hash_line_number_to_index_dict.keys():
                    CVE_dict_line_number[CVE].append(file_hash_line_number_to_index_dict[ele+"__split__"+str(CVE_dict[CVE][ele][name]["lineEnd"])])
    zero_CVE_set=set()
    ...
    for CVE in filtered_dict:
        ...
        match_dict={}
        for key in sig.keys():
            if key.count("__split__")<=1 and not (key.startswith("del__split__") or key.startswith("add__split__")):
                logger.info(CVE + " " + key)
