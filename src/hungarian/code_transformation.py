...
def operator_extraction(st, code: str, language: Language):
    ...
    query = "(binary_expression)@name (unary_expression) @name (assignment_expression)@name"
    while 1:
        ...
        for node in nodes:
            ...
            if op in arithmetic_operator:
                ...
                code_bytes = code_bytes[:start_byte] + new_express.encode() + code_bytes[end_byte:]
                finished = False
                break
...
def del_DeadCode(st, code: str, language: Language):
    ...
    while True:
        parser = ASTParser(code_bytes.decode(), language)
        nodes = parser.query_all("(while_statement) @name (if_statement) @name (for_statement) @name")
        finished = True
        for node in nodes:
            condition = node.child_by_field_name('condition')
            if condition is None:
                continue
            if condition and condition.text.decode() == '(0)' or condition.text.decode() == '0':
                start_byte = node.start_byte
                end_byte = node.end_byte
    ...
    while True:
        ...
        for node in nodes:
            if node.type == 'for_statement':
                ...
                increment_code = increment.text.decode() if increment is not None else ""
                while_loop = f"{init_code}\nwhile ({condition_code}){{\n "
                while_loop += body.text.decode()[1:-1]
                while_loop += f"\n{increment_code};\n}}\n"
                start_byte = node.start_byte
                end_byte = node.end_byte
                code_bytes = code_bytes[:start_byte] + while_loop.encode() + code_bytes[end_byte:]
                finished = False
                break
            else:
                continue
...
def astyle(code: str) -> str:
    code = subprocess.run(['astyle', '--style=java', '--squeeze-ws', '--keep-one-line-statements', '--delete-empty-lines',
...
def move_definition(st, code: str, language: Language):
    ...
    while True:
        ...
        for node in nodes:
            ...
            code_bytes = code_bytes[:start_byte] + reconstruct_codes + code_bytes[end_byte:]
            break
        if finished:
            break
        ...
        for node in nodes:
            ...
            for declarator in declarators:
                if declarator.type != "init_declarator":
                    ...
                    continue
                identifier = parser.query_all("(identifier)@name", node=declarator)[0].text.decode()
            ...
            if has_init:            
                ...
                code_bytes = code_bytes[:start_byte] + new_code.encode() + code_bytes[end_byte:]
                finished = False
                break
        if finished:
            break
...
def code_transformation(codes:str, language:Language): 
    ...
    # results = cpu_heater.multithreads(del_DeadCode, codes_split,  max_workers=1024, show_progress=False)
    # results = cpu_heater.multithreads(while_for_transformation, results,  max_workers=1024, show_progress=False)
    results = cpu_heater.multithreads(operator_extraction, codes_split, max_workers=1024, show_progress=False)
    # results = cpu_heater.multithreads(operator_extraction, results, max_workers=1024, show_progress=False)
    # results = cpu_heater.multithreads(results, move_definition, max_workers=8192, show_progress=True)
    codes = ""
...
def abstract(code: str, language:Language):    
    ...
    for param in fparams:
        ...
        try:
            ...
            if fparams[param] != "":
                abstractBody = paramPattern.sub(f"\g<1>{fparams[param]}_FPARAM_{cnt}\g<2>", abstractBody)
            else:            
    ...
    for lvar in lvars:
        ...
        try:
            ...
            if lvars[lvar] != "":
                abstractBody = lvarPattern.sub(f"\g<1>{lvars[lvar]}_LVAR_{cnt}\g<2>", abstractBody)
            else:            
    ...
    for string in strings:
        ...
        try:
            ...
            if len(fmt_list) != 0:
                ...
                abstractBody = fstrPattern.sub(r"\g<1>" + f"{replace_str}_{cnt}" + r"\g<2>", abstractBody)
            else:
                replace_str = "STRING"
                fstrPattern = re.compile(r"(^|\W)" + re.escape(string) + r"(\W)")
                abstractBody = fstrPattern.sub(r"\g<1>" + f"{replace_str}_{cnt}" + r"\g<2>", abstractBody)
    ...
    removedMTags = ["__FILE__", "__LINE__", "__DATE__", "__TIME__", "__STDC__", "__STDC_VERSION__", 
                "__cplusplus", "__GNUC__", "__GNUC_MINOR__", "__GNUC_PATCHLEVEL__", "__BASE_FILE__", "__FILE_NAME__", 
                "__INCLUDE_LEVEL__", "__VERSION__","__CHAR_UNSIGNED__", "__WCHAR_UNSIGNED__","__REGISTER_PREFIX__", "__USER_LABEL_PREFIX__", "const", "static", "extern"]
...
def extraction_marcos_and_format(codes: str, repoName: str, fileName: str):
    removedMacros = ["__FILE__", "__LINE__", "__DATE__", "__TIME__", "__STDC__", "__STDC_VERSION__", 
                "__cplusplus", "__GNUC__", "__GNUC_MINOR__", "__GNUC_PATCHLEVEL__", "__BASE_FILE__", "__FILE_NAME__", 
                "__INCLUDE_LEVEL__", "__VERSION__","__CHAR_UNSIGNED__", "__WCHAR_UNSIGNED__","__REGISTER_PREFIX__", "__USER_LABEL_PREFIX__", "__attribute__((noreturn))"]
    ...
    while not gcc_finish:
        try:            
            cmd = "gcc -E -w -include \"" + targetRepoMacros + "/{0}/macro_{1}.h\" \"{2}\" -o \"{3}\"".format(repoName, pure_fileName, temp_source_file_name, temp_output_file_name)
            # print(cmd)
            ...
            while i < len(msgs):
                ...
                if match:
                    ...
                    if not (targetRepoMacros not in fileName or pure_fileName not in fileName) or fileName.startswith("/"):
                        i += 1
                        continue
    ...
    with open(temp_source_file_name, "r", encoding=encoding_format) as f:
        ...
        while i < len(lines):
            if (
                ...
                if lines[preTemp].strip().endswith("{"):
                    ...
                    lines[i] = "\n"
            elif lines[i].strip().startswith("||") or lines[i].strip().startswith("&&") or lines[i].strip().startswith(">>") or lines[i].strip().startswith(")") or (lines[i].strip().startswith("(") and not lines[preTemp].strip().endswith("{") and not (lines[preTemp].strip().endswith(";") and not lines[preTemp].strip().startswith("for"))):
        ...
        while i < len(lines):
            ...
            if temp_source_file_name in lines[i]:
                ...
                if lines[i].replace(" ","")[j + len(temp_source_file_name)+1]=="," and lines[i].replace(" ","")[j + len(temp_source_file_name)+2].isdigit():
                    k = lines[i].replace(" ","")[j + len(temp_source_file_name)+2:].find(",")
                    if k != -1:
                        digit = lines[i].replace(" ","")[j + len(temp_source_file_name)+2:j + len(temp_source_file_name)+2+k]
                    else:
                        k = lines[i].replace(" ","")[j + len(temp_source_file_name)+2:].find(")")
                        digit = lines[i].replace(" ","")[j + len(temp_source_file_name)+2:j + len(temp_source_file_name)+2+k]
                    # print(digit)
                    lines[i] = lines[i].replace(temp_source_file_name + ",","").replace(digit + ",", "").replace(temp_source_file_name,"").replace(digit,"")
        ...
        if flag:
            ...
            while not getInclude and prefix != "/".join(repoPath.split("/")[:-1]):
                ...
                try:
                    ...
                    functionList = subprocess.check_output(CTAGS_PATH + ' -f - --kinds-C=* --fields=neKSt "' + temp_source_file_name + '"', stderr=subprocess.STDOUT, shell=True).decode(errors='replace')
...
def extraction_macros(code: str, repo_path: str, fileName: str, commit:str="", all_blobs={}):
    repoName = repo_path.split("/")[-1]
    if not os.path.exists(targetRepoMacros + "/{0}/macro_{1}.h".format(repoName, fileName.replace("/","_") + "_" + commit)):
    ...
    return extraction_marcos_and_format(code, repoName, fileName.replace("/","_") + "_" + commit)


## 顺序不建议更换！！！！
def abstraction(code: str, language:Language):
    codes = code_transformation(code, Language.CPP)
    # print("finish code transformation...")
    # fp = open("temp_1.c", "w")
    # fp.write(codes)
    # fp.close()
...
if __name__ == "__main__":
    ...
    fp = open("/home/cyh/llm4vuln/2.methodology/test_before.c")
    lines = fp.read()
    fp.close()
    # codes = format_code.format_and_del_comment_c_cpp(lines)
    source_code_before = code_transformation(lines, Language.CPP)
    # codes = format_and_del_comment_c_cpp(codes)
    fp = open("/home/cyh/llm4vuln/2.methodology/test.c", "w")
    fp.write(source_code_before)
    fp.close()
    # fp = open("/home/cyh/llm4vuln/1.groundtruth/signature_repo_cache/389ds@@389-ds-base/ldap/servers/slapd/back-ldbm/cache.c")
    # lines = fp.read()
    # fp.close()

    # # print()

    # codes: None | str = extraction_macros(lines, "/home/cyh/llm4vuln/1.groundtruth/signature_repo_cache/389ds@@389-ds-base", "/home/cyh/llm4vuln/1.groundtruth/signature_repo_cache/389ds@@389-ds-base/ldap/servers/slapd/back-ldbm/cache.c")
    # print("finish macro extraction...")
    # assert codes is not None
    # fp = open("temp.c", "w")
    # fp.write(code_transformation(codes, Language.CPP))
    # fp.close()
